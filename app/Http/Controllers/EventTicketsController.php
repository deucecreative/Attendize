...
class EventTicketsController extends MyBaseController
{
    ...
    public function showTickets(Request $request, $event_id)
    {
        ...
        $tickets = empty($q) === false
            ? $event->tickets()->where('title', 'like', '%' . $q . '%')->orderBy($sort_by, 'asc')->paginate(config('attendize.default_items_per_page'))
            : $event->tickets()->orderBy($sort_by, 'asc')->paginate(config('attendize.default_items_per_page'));
